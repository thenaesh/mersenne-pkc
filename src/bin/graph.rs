...
fn main() {
    ...
    for h in hs.iter() {
        for n in ns.iter() {
            println!("n = {}, h = {}", *n, *h);
            let (ex_mean, ex_var, unex_max_mean, unex_max_var, ex_max_mean, ex_max_var) = generate_and_plot_frequencies(*n, *h);
...
fn generate_and_plot_frequencies(n: usize, h: usize) -> (f64, f64, f64, f64, f64, f64) {
    ...
    for i in 0..10000 {
        let (xs, y, z) = generate_reductions(n, h);
    ...
    println!("{} expected points, {} unexpected max points, {} expected max points",
    ...
    plot_frequencies(&format!("graphs/freq_plot_{}_{}.svg", n.to_string(), h.to_string()), &expected_points, &unexpected_max_points, &expected_max_points);
...
fn plot_frequencies(output_filename: &str, expected_points: &Vec<(f64, f64)>, unexpected_max_points: &Vec<(f64, f64)>, expected_max_points: &Vec<(f64, f64)>) {
    ...
    println!("{:?}", unexpected_max_points);
    println!("{:?}", expected_max_points);
    ...
    let s2 = Scatter::from_vec(unexpected_max_points.as_slice())
        .style(scatter::Style::new() // uses the default marker
            .colour("#FF0000")); // and a different colour

    let s3 = Scatter::from_vec(expected_max_points.as_slice())
        .style(scatter::Style::new() // uses the default marker
            .colour("#0000FF")); // and a different colour
...
fn generate_reductions(n: usize, h: usize) -> (Vec<i64>, i64, i64) {
    let (f, g) = randomly_generate_message(n, h);
    ...
    (expected_point_hamming_weight_reductions, unexpected_points_max_hamming_weight_reduction, expected_points_max_hamming_weight_reduction)
}
