...
fn main() {
    ...
    for h in hs.iter() {
        for n in ns.iter() {
            println!("n = {}, h = {}", *n, *h);
            let (ex_mean, ex_var, unex_mean, unex_var) = generate_and_plot_frequencies(*n, *h);
            println!("Expected Points: mean = {}, stddev = {}", ex_mean, ex_var.sqrt());
            println!("Unxpected Points: mean = {}, stddev = {}", unex_mean, unex_var.sqrt());
        }
    }
...
fn generate_and_plot_frequencies(n: usize, h: usize) -> (f64, f64, f64, f64) {
    let mut expected_frequency_map = HashMap::<i64, usize>::new();
    let mut unexpected_frequency_map = HashMap::<i64, usize>::new();

    for i in 0..2500 {
        let (xs, y) = generate_reductions(n, h);

        unexpected_frequency_map.insert(y, match unexpected_frequency_map.get(&y) {
            Some(a) => a + 1,
            None => 1,
        });
    ...
    let unexpected_points: Vec<(f64, f64)> = unexpected_frequency_map.iter().map(|(hw, freq)| (*hw as f64, *freq as f64)).collect();

    println!("{} expected points, {} unexpected points", expected_points.iter().fold(0., |acc, (_, n)| acc + n), unexpected_points.iter().fold(0., |acc, (_, n)| acc + n));

    plot_frequencies(&format!("graphs/freq_plot_{}_{}.svg", n.to_string(), h.to_string()), &expected_points, &unexpected_points);
    ...
    let unexpected_points_sample_mean = {
        let (sum, n) = unexpected_points.iter().fold((0., 0.), |(sum, n), (hw, freq)| (sum + *hw * *freq, n + *freq));
        sum / n
    };
    let unexpected_points_sample_variance = {
        let (var, n) = unexpected_points.iter().fold((0., 0.), |(sumsq, n), (hw, freq)| {
            let delta = *hw - expected_points_sample_mean;
            (sumsq + freq * delta * delta, n + freq)
        });
        var / (n - 1.)
    ...
    (expected_points_sample_mean, expected_points_sample_variance, unexpected_points_sample_mean, unexpected_points_sample_variance)
}
...
fn plot_frequencies(output_filename: &str, expected_points: &Vec<(f64, f64)>, unexpected_points: &Vec<(f64, f64)>) {
    ...
    let (unexpected_max_hw, unexpected_max_freq) = unexpected_points.iter()
        .fold((0. as f64, 0. as f64), |(acc_hw, acc_freq), (x, y)| (acc_hw.max(*x), acc_freq.max(*y)));
...
fn generate_reductions(n: usize, h: usize) -> (Vec<i64>, i64) {
    let (f, g) = randomly_generate_message(n, h);
    ...
    (expected_point_hamming_weight_reductions, unexpected_points_max_hamming_weight_reduction)
}
